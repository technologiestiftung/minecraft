...
NBT.Tag.prototype.getBuffer = function() {
  ...
  switch (this.type) {
    case NBT.BYTE:
      ...
      this.putByte(this.value);
      return;

    case NBT.SHORT:
      var buffer = new Buffer(2);
      buffer.writeInt16BE(this.value, this.offset);
      this.offset += 2;
      return;

    case NBT.INT:
      var buffer = new Buffer(4);
      buffer.writeInt32BE(this.value, this.offset);
      this.offset += 4;
      return;
    ...
    case NBT.LONG:
      ...
      buffer.writeInt32BE(hi, this.offset);
      this.offset += 4;

      var lo = this.value.getLowBits();
      buffer.writeInt32BE(lo, this.offset);
      this.offset += 4;
      return;

    case NBT.FLOAT:
      var buffer = new Buffer(4);
      buffer.writeFloatBE(this.value, this.offset);
      this.offset += 4;
      return;

    case NBT.DOUBLE:
      var buffer = new Buffer(8);
      buffer.writeDoubleBE(this.value, this.offset);
      this.offset += 8;
      return;
    ...
    case NBT.BYTE_ARRAY:
       ...
       buffer.writeInt32BE(length, this.offset);
       this.offset += 4;
       for (var i = 0; i < length; i++) {
         this.putByte(this.value[i]);
       }
      return;
...
//    var buffer = new Buffer(4);
    ...
    case NBT.INT_ARRAY:
       ...
       this.offset += 4;
       for (var i = 0; i < length; i++) {
         buffer.writeInt32BE(this.value[i], this.offset);
         this.offset += 4;
       }
      return;
  }
