...
Region.fromFile = function(file) {
  ...
  var
    b1, b2, b3, b4,
    chunkOffset, chunkTime, chunkLength, compressionType, compressedStr,
    chunk;
	...
	for (var z = 0; z < 32; z++) {
        ...
        continue;
			...
			compressedStr = region.buffer.toString('utf8', (chunkOffset<<12) + 5, chunkLength-1);

			var chunk;
			if (compressionType === 1) {
        chunk = zlib.gunzipSync(compressedStr);
			} else if (compressionType === 2) {
        throw ('+++ COMPERSSION TYPE 2 +++');
				// chunk = zlib.uncompress(compressedStr);
			} else {
				throw ('Unknown compression type ' + compressionType);
			}

			region[z][x].data = new NBT(chunk);
...
Region.prototype.setBlock = function(x, y, z, type) {
  ...
  section.value.Blocks.value[offset] = type;
	...
	// set subtype
	var byte = section.value.Data.value.charCodeAt(offset/2);
	if (offset % 2 === 0 ) {
...
Region.prototype.getBlockSection = function(x, y, z) {
  ...
  if (this.chunks[c.z]) {
	...
	var sectionY = y/16 <<0;
	var sections = chunk.data.value.Level.value.Sections;
...
Region.prototype.addChunk = function(x, z) {
  ...
  this.chunks[z][x] = {
		timestamp: Date.now(),
		data: NBT.createTag(NBT.COMPOUND, '', { Level:level })
	};
...
Region.prototype.getBuffer = function() {
	...
	for (var z = 0; z < 32; z++)	{
		for (var x = 0; x < 32; x++) {
 			...
 			var chunkNBT = this.chunks[z][x].data.getBuffer();
