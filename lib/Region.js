/* global 4096 */

var fs = require('fs');
var zlib = require('zlib');

var NBT = require('./NBT.js');

//*****************************************************************************

function arrayFill(v, num) {
  var res = [];
  for (var i = 0; i < num; i++) {
    res[i] = v;
  }
  return res;
}

//*****************************************************************************

var Region = module.exports = function(buffer) {
	this.buffer = buffer || new Buffer(1024*1024); // TODO
	this.offset = 0;
  this.chunks = {};
};

//*****************************************************************************

Region.fromFile = function(file) {
  var region = new Region(fs.readFileSync(file));

  var
    b1, b2, b3, b4,
    chunkOffset, chunkTime, chunkLength, compressionType, compressedBuffer,
    chunk;

	for (var z = 0; z < 32; z++) {
		for (var x = 0; x < 32; x++) {
			b1 = region.getByte();
			b2 = region.getByte();
			b3 = region.getByte();
			b4 = region.getByte();
			chunkOffset = (b1<<16) + (b2<<8) + (b3);

			if (chunkOffset === 0) {
        continue;
      }

 			region.offset = 4*32*32 + 4 * (x + z * 32);
			chunkTime = region.getInt32();

			region.offset = chunkOffset<<12; // <<12 = 4096
			chunkLength = region.getInt32();
			compressionType = region.getByte();
      var start = (chunkOffset<<12) + 5;
      var end = start + chunkLength-1;
			compressedBuffer = region.buffer.slice(start, end);

			var chunk;
			if (compressionType === 1) {
        chunk = zlib.gunzipSync(compressedBuffer);
			} else if (compressionType === 2) {
				chunk = zlib.unzipSync(compressedBuffer);
			} else {
				throw ('Unknown compression type ' + compressionType);
			}

			if (region[z] === undefined) {
        region[z] = {};
      }

			region[z][x] = new NBT(chunk);
			region[z][x].timestamp = chunkTime;
		}
	}

	return region;
};

//*****************************************************************************

Region.prototype.getByte = function() {
  var byte = this.buffer.readUInt8(this.offset);
  this.offset += 1;
  return byte;
};

Region.prototype.getInt32 = function() {
	var t1 = this.getByte();
	var t2 = this.getByte();
	var t3 = this.getByte();
	var t4 = this.getByte();
	return (t1<<24) + (t2<<16) + (t3<<8) + t4;
};

Region.prototype.addLayer = function(y, type) {
 	for (var z = 0; z < 512; z++) {
 		for (var x = 0; x < 512; x++) {
 			this.setBlock(x, y, z, type);
 		}
 	}
};

Region.prototype.setBlock = function(x, y, z, type) {
	var subtype = 0;

  if (parseInt(type, 10) !== type) {
    var t = type.split('.');
		type = parseInt(t[0], 10);
    subtype = t[1] === '1' ? 10 : parseInt(t[1], 10);
	}

	if (type !== (type & 255)) {
    throw('bad type passed to setBlock: ' + type);
  }

	if (subtype !== (subtype & 15) ) {
    throw('bad subtype passed to setBlock: ' + subtype);
  }

  var section = this.getBlockSection(x, y, z);
	if (section === undefined) {
		section = this.addSection(x, y, z);
	}

	var offset = this.getBlockOffset(x, y, z);
  section.value.Blocks.value[offset] = type;

	// set subtype
	var byte = section.value.Data.value[offset/2];
	if (offset % 2 === 0 ) {
		byte = (byte & 240) + subtype;
	} else {
		byte = subtype * 16 + (byte & 15);
	}

  section.value.Data.value[offset/2] = byte;

	this.changed = true;
};

Region.prototype.getBlockSection = function(x, y, z) {
  var c = this.getChunkXZ(x, z);
  if (this.chunks[c.z]) {
	  var chunk = this.chunks[c.z][c.x];
  }

	if (chunk === undefined) {
    return;
  }

	var sectionY = y/16 <<0;
	var sections = chunk.value.Level.value.Sections;
	return sections[sectionY];
};

Region.prototype.getChunkXZ = function(x, z) {
	return { x:x/16 <<0, z:z/16 <<0 };
};

Region.prototype.addSection = function(x, y, z) {
  var c = this.getChunkXZ(x, z);

	if (this.chunks[c.z] === undefined || this.chunks[c.z][c.x] === undefined) {
		this.addChunk(c.x, c.z);
	}

	var chunk = this.chunks[c.z][c.x];
	var sections = chunk.value.Level.value.Sections;

	var sectionY = y/16 <<0;

	for (var i = 0; i <= sectionY; i++) {
		if (sections.value[i] !== undefined) {
      continue;
    }

    var value = {
		  Y:          NBT.createTag(NBT.BYTE,       'Y',          i),
		  Blocks:     NBT.createTag(NBT.BYTE_ARRAY, 'Blocks',     arrayFill(0, 4096)),
		  BlockLight: NBT.createTag(NBT.BYTE_ARRAY, 'BlockLight', arrayFill(0, 2048)),
		  SkyLight:   NBT.createTag(NBT.BYTE_ARRAY, 'SkyLight',   arrayFill(255, 2048)),
		  Data:       NBT.createTag(NBT.BYTE_ARRAY, 'Data',       arrayFill(0, 2048))
    };

		sections.value[i] = NBT.createTag(NBT.COMPOUND, null, value);
	}

	this.changed = true;

	return sections.value[i-1];
};

Region.prototype.addChunk = function(x, z) {
  var value = {
    TerrainPopulated: NBT.createTag(NBT.BYTE,       'TerrainPopulated', 1),
    Biomes:           NBT.createTag(NBT.BYTE_ARRAY, 'Biomes',           arrayFill(0, 256)),
    xPos:             NBT.createTag(NBT.INT,        'xPos',             x),
    zPos:             NBT.createTag(NBT.INT,        'zPos',             z),
    LightPopulated:   NBT.createTag(NBT.BYTE,       'LightPopulated',   0),
    Entities:         NBT.createTag(NBT.LIST,       'Entities',         [], 10),
    TileEntities:     NBT.createTag(NBT.LIST,       'TileEntities',     [], 10),
    LastUpdate:       NBT.createTag(NBT.LONG,       'LastUpdate',       0),
    HeightMap:        NBT.createTag(NBT.INT_ARRAY,  'HeightMap',        []),
    Sections:         NBT.createTag(NBT.LIST,       'Sections',         [], 10)
  };

	var level = NBT.createTag(NBT.COMPOUND, 'Level', value);

  if (this.chunks[z] === undefined) {
    this.chunks[z] = {};
  }

  // TODO: tag is not a full NBT object yet. it needs at least a getBiuffer() Method
  this.chunks[z][x] = NBT.createTag(NBT.COMPOUND, '', { Level:level });
  this.chunks[z][x].timestamp = Date.now();

	this.changed = true;
};

Region.prototype.getBlockOffset = function(x, y, z) {
 	var localX = x & 15;
  var localY = y & 15;
 	var localZ = z & 15;
 	return 16*16*localY + 16*localZ + localX;
};

Region.prototype.getBuffer = function() {


  return;

	var chunkOffset = 2;
	var offset = [];
	var timeData = [];
	var chunkData = [];

	// turn each chunk into data
	for (var z = 0; z < 32; z++)	{
		for (var x = 0; x < 32; x++) {
 			if (this.chunks[z] === undefined) {
 				this.chunks[z] = {};
 			}

 			if (this.chunks[z][x] === undefined) {
 				offset.push(0, 0, 0, 0);
 				timeData.push(0, 0, 0, 0);
 				continue;
 			}

 			var chunkTime = this.chunks[z][x].timestamp;
 			var chunkNBT = this.chunks[z][x].getBuffer();
   		var compressedStr = zlib.gzipSync(chunkNBT);
 			var length = compressedStr.length + 1;

 			var chunk = ((length >>24) & 255) + ((length >>16) & 255) + ((length >>8) & 255) + ((length) & 255) + 2;
 			chunk += compressedStr;

 			if (chunk.length % 4096 !== 0) {
        var num = 4096 - (chunk.length % 4096);
        for (var i = 0; i < num; i++) {
// TODO this should be handled as buffer / array
          chunk += 0;
        }
 			}

 			if (chunk.length % 4096 !== 0) {
 				throw('Math fail');
 			}

 			var sectors = chunk.length/4096;

 			chunkData.push(chunk);
 			offset.push(((chunkOffset >>16) & 255) + ((chunkOffset >>8) & 255) + ((chunkOffset)   & 255) + (sectors));
 			timeData.push(((chunkTime >>24) & 255) + ((chunkTime  >>16) & 255) + ((chunkTime >>8) & 255) + ((chunkTime) & 255));
 			chunkOffset += sectors;
 		}
 	}

 	return [offset, timeData, chunkData].join('');
};

Region.prototype.toFile = function(file) {
////
return

  fs.writeFileSync(file, this.getBuffer());
};
